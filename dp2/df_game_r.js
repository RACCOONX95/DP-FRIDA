    ...
    Interceptor.attach(ptr(0x8652F0C),
        {
            onEnter: function (args)
            {
                ...
                if(login_tick > 0)
                {
                    ...
                    //发消息通知客户端奖励已发送
                    api_CUser_SendNotiPacketMessage(user, '[' + get_timestamp() + '] 在线奖励已发送(当前阶段:' + reward_cash_cera + ')', 6);
                }
            },
            onLeave: function (retval)
            {
            ...
            onEnter: function(args)
            {
                ...
                else if (game_event == 'DungeonClearInfo') //副本信息清除
                {
                    ...
                    //{
                        ...
                        {
                            api_gameWorld_SendNotiPacketMessage(identification + api_CUserCharacInfo_getCurCharacName(user) + ']' + '通关' + dgnname[charac_no] + '用时 ' + parseInt((savedFirstSecondsValue / 60)) + '分' + (savedFirstSecondsValue % 60) + '秒', 14);
                            //console.log(identification + api_CUserCharacInfo_getCurCharacName(user) + ']' + '通关' + dgnname[charac_no] + '用时 ' + parseInt((savedFirstSecondsValue / 60)) + ' 分 ' + (savedFirstSecondsValue % 60) + ' 秒');
                            clearSavedFirstSecondsValue(charac_no);
                        }
                        /!*else if (savedFirstSecondsValue < 5) //如果副本通关时间小于5秒则踢出游戏
                        {
                        ...
                        else
                        {
                            ...
                            {
                                api_gameWorld_SendNotiPacketMessage(identification + api_CUserCharacInfo_getCurCharacName(user) + ']' + '未通关' + dgnname[charac_no] + '并离开副本，用时 ' + parseInt((seconds / 60)) + '分' + (seconds % 60) + '秒', 14);
                        ...
                        //api_GameWorld_SendNotiPacketMessage(api_CUserCharacInfo_getCurCharacName(user) + '（' + dgnname[charac_no] + '）通关时间：' + parseInt((savedFirstSecondsValue / 60)) + '分' + (savedFirstSecondsValue % 60) + '秒', 14);
                        //clearSavedFirstSecondsValue(charac_no);
                    //}
                }
                ...
                {
                    api_GameWorld_SendNotiPacketMessage('['+get_timestamp()+']<'+api_CUserCharacInfo_getCurCharacName(user)+'>升级Lv.'+charac_level,14)
                }
                else if (game_event == 'Item+')
                {
                    api_CUser_SendNotiPacketMessage(user, 'GAME_EVENT：' + game_event + '=>'+ api_CItem_GetItemName(item_id) +'['+ item_id +']', 1);
                }
                else
                {
                    api_CUser_SendNotiPacketMessage(user, 'GAME_EVENT：' + game_event, 1);
                }
...
function Top_up_Lucky_Points(user) {
    ...
    //通知客户端当前角色幸运点已改变
    api_CUser_SendNotiPacketMessage(user, '你就是最幸运的人!', 0);
}
//--------------------------------------------------------------------
    ...
    Interceptor.attach(ptr(0x86C4E50),
        {
            ...
            onLeave: function(retval)
            {
                ...
                //给角色发消息问候
                //api_CUser_SendNotiPacketMessage(this.user, 'XXXX ' + api_CUserCharacInfo_getCurCharacName(this.user), 2);
                api_GameWorld_SendNotiPacketMessage('['+get_timestamp()+']欢迎<'+ 'Lv.' +CUserCharacInfo_get_charac_level(this.user)+' '+api_CUserCharacInfo_getCurCharacName(this.user) + '>上线', 14);
            }
        });
